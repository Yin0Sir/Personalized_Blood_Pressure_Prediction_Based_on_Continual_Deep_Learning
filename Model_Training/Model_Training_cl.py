...
if __name__ == '__main__':
    ...
    # model.to(device)
    # Settings = {'BP_optimizer': 'torch.optim.Adam(model.parameters(), lr=0.001, betas=(0.9, 0.999), weight_decay=0)',        #  从零开始训练
    ...
    Settings = {
        'BP_optimizer': "torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-3, betas=(0.9, 0.999), weight_decay=0)",     # 加载预训练模型
    ...
    # 加载预训练模型
    pretrained_model_path = 'PTH/132036/trained_model.pth'
    ...
    layers_to_unfreeze = ["final_fc"]  # 综合最好的结果,"resnet.layer4", "resnet.layer3"
