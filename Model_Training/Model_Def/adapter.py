...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_BP=1, zero_init_residual=False, norm_layer=nn.BatchNorm1d):
        ...
        self.layer1_cf = CFBlock(64)
        self.layer2_cf = CFBlock(64,downsample=True)
        self.layer3_cf = CFBlock(128,downsample=True)
        self.layer4_cf = CFBlock(256,downsample=True)
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer1(x) + self.layer1_cf(x)
        x = self.layer2(x) + self.layer2_cf(x)
        x = self.layer3(x) + self.layer3_cf(x)
        x = self.layer4(x) + self.layer4_cf(x)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        # x = self.fc(x)
        return x
...
class BPRegressionHead(nn.Module):
    def __init__(self, input_dim=128):
        super().__init__()
        self.output = nn.Linear(input_dim, 1)  # Predict SBP, DBP

    def forward(self, x):
        return self.output(x)

class PersonalizedBPModel(nn.Module):
    def __init__(self, backbone, adapter_dict, output_head):
        super().__init__()
    ...
    def forward(self, x, uid):
        ...
        if uid not in self.adapter_dict:
            self.adapter_dict[uid] = Adapter(input_dim=features.shape[1])
        adapted = self.adapter_dict[uid](features)
        return self.output_head(adapted)
