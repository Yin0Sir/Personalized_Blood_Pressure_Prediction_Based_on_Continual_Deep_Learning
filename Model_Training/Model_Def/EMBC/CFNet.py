...
class CFBlock(nn.Module):
    def __init__(self, in_channels, num_heads=8, drop_rate=0, drop_path_rate=0):
...
class BasicBlock(nn.Module): #BasicBlock 始终具有 dilation=1 和 groups=1
    ...
    def __init__(self, in_channels, out_channels, stride = 1, downsample = None, norm_layer = nn.BatchNorm1d,):
        ...
        self.stride = stride
        self.cf_block = CFBlock(out_channels)  # 添加 SE 模块
...
if __name__ == '__main__':
    ...
    input = torch.randn(32, 32, 1250).to(device)  # (batch_size, channels, time_steps)
