...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, norm_layer=nn.BatchNorm1d):
        ...
        # 中间通道数，确保输出通道数为 out_channels
        med_channels = out_channels // self.expansion
        self.conv1 = conv1x1(in_channels, med_channels)
        self.bn1 = norm_layer(med_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x1(med_channels, med_channels, stride=stride)
        self.bn2 = norm_layer(med_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv3 = conv1x1(med_channels, out_channels * self.expansion)
        self.bn3 = norm_layer(out_channels * self.expansion)
        self.stride = stride
        # 自动判断是否需要下采样，确保输入和输出的形状一致
        if downsample is None and (in_channels != out_channels * self.expansion * self.expansion or stride != 1):
