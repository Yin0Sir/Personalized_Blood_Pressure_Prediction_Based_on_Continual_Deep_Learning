...
class DC1_ECG(nn.Module):
    def __init__(self, kernel_size=3, num_groups=8, bias=True, reduction=4):
        super().__init__()
        self.independent_conv1 = DynamicConv1d(1, 64, kernel_size, num_groups, bias)
    ...
    def forward(self, x):
        ...
        x = self.independent_conv1(x[:, 0:1, :])
        ecg_features = self.norm(x)
        ecg_features = self.activation(ecg_features)

        return ecg_features
    
class DC1_PPG(nn.Module):
    def __init__(self, kernel_size=3, num_groups=8, bias=True, reduction=4):
        super().__init__()
        self.independent_conv1 = DynamicConv1d(1, 64, kernel_size, num_groups, bias)
        self.norm = nn.BatchNorm1d(64)
    ...
    def forward(self, x):
        B, C, L = x.shape
        assert C == 2, "Input must have 2 channels (PPG and ECG)."
        x = self.independent_conv1(x[:, 1:2, :])
        ppg_features = self.norm(x)
        ppg_features = self.activation(ppg_features)
...
class DC2(nn.Module):
    ...
    def forward(self, x):
        B, C, L = x.shape
        assert C == 2, "Input must have 2 channels (PPG and ECG)."
        joint_features = self.joint_conv(x)
        joint_features = self.mse_block(joint_features)
        joint_features = self.norm(joint_features)
        joint_features = self.activation(joint_features)

        return joint_features

class DC2_1(nn.Module):
    def __init__(self, kernel_size=3, num_groups=8, bias=True, reduction=4):
        super().__init__()
    ...
    def forward(self, x):
        B, C, L = x.shape
        assert C == 2, "Input must have 2 channels (PPG and ECG)."
        x1 = self.independent_conv1(x[:, 0:1, :])
        x2 = self.independent_conv2(x[:, 1:2, :])
        independent_features = torch.cat([x1, x2], dim=1)
        independent_features = self.norm(independent_features)
        independent_features = self.activation(independent_features)
...
class ResNet1_PPG(nn.Module):
    def __init__(self, block, layers, num_BP=1, zero_init_residual=False, norm_layer=nn.BatchNorm1d):
        super(ResNet1_PPG, self).__init__()
        self._norm_layer = norm_layer
...
class CombinedCorNetResNet1_ECG(nn.Module):
    def __init__(self, cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1):
        super(CombinedCorNetResNet1_ECG, self).__init__()
        
        # 将 ResNet 初始化为特征提取器
        self.resnet = ResNet1_ECG(block=BasicBlock, layers=[3, 4, 6, 3], num_BP=num_BP)
        # 初始化 CorNet 以进行上下文增强
...
class CombinedCorNetResNet1_PPG(nn.Module):
    def __init__(self, cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1):
        super(CombinedCorNetResNet1_PPG, self).__init__()
        
        # 将 ResNet 初始化为特征提取器
        self.resnet = ResNet1_PPG(block=BasicBlock, layers=[3, 4, 6, 3], num_BP=num_BP)
        # 初始化 CorNet 以进行上下文增强
        self.cornet = CorNet(output_size=cornet_output_size, cornet_dim=cornet_dim, n_cornet_blocks=n_cornet_blocks)
        # 添加最终线性层以将 CorNet 输出映射到单个 BP 预测
        self.final_fc = nn.Linear(cornet_output_size, num_BP)
...
class CombinedCorNetResNet2(nn.Module):
    def __init__(self, cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1):
        super(CombinedCorNetResNet2, self).__init__()
        self.resnet = ResNet2(block=BasicBlock, layers=[3, 4, 6, 3], num_BP=num_BP)
        self.cornet = CorNet(output_size=cornet_output_size, cornet_dim=cornet_dim, n_cornet_blocks=n_cornet_blocks)
        self.final_fc = nn.Linear(cornet_output_size, num_BP)

    def forward(self, x):
        resnet_features = self.resnet._forward_impl(x)
        cornet_output = self.cornet(resnet_features)
        output = self.final_fc(cornet_output)
        
        return output
    
class CombinedCorNetResNet2_1(nn.Module):
    def __init__(self, cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1):
        super(CombinedCorNetResNet2_1, self).__init__()
        self.resnet = ResNet2_1(block=BasicBlock, layers=[3, 4, 6, 3], num_BP=num_BP)
        self.cornet = CorNet(output_size=cornet_output_size, cornet_dim=cornet_dim, n_cornet_blocks=n_cornet_blocks)
        self.final_fc = nn.Linear(cornet_output_size, num_BP)
...
def DCCR_1ECG():
    return CombinedCorNetResNet1_ECG(cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1)
def DCCR_1PPG():
    return CombinedCorNetResNet1_PPG(cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1)
def DCCR_2():
    return CombinedCorNetResNet2(cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1)
def DCCR_2_1():
    return CombinedCorNetResNet2_1(cornet_output_size=512, cornet_dim=1000, n_cornet_blocks=2, num_BP=1)
