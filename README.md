# CLBP：基于持续深度学习的个性化血压预测研究

本仓库为硕士课题《**基于持续深度学习的个性化血压预测研究**》的项目代码。项目围绕 ECG/PPG 双通道时序信号，构建基线血压预测模型，并在个体连续数据流上比较多种持续学习个性化策略（Fine-tune / EWC / Replay / Hybrid）。
...
- **全局模型能力**：`Model_Training.py`
- **个性化微调能力**：`Fine_tune.py`
- **持续学习能力（抗遗忘）**：`Model_Training_CL.py` + `Trainer.py` 中 `EWCRegularizer` / `ReplayBuffer`
- **临床可解释评估图**：`Model_eval.py`

---

如你愿意，我可以下一步继续帮你补一个“论文可直接引用”的实验设置章节（数据划分、超参数表、评价指标定义、统计检验方案）版本 README 附录。
